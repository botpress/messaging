...
const Form: FC<FormProps> = ({
  ...
  const getArrayPlaceholder = (index, field) => {
    ...
    return index === 0 && placeholder ? lang.tr(placeholder) : ''
  }
        ...
        return (
          <Fragment key={field.key}>
            ...
            {(!defaultLang || defaultLang === currentLang) && (
              <AddButton
                text={lang.tr(field.group?.addLabel)}
                onClick={() => {
        ...
        return (
          <FieldWrapper key={field.key} label={printLabel(field, currentValue, parent, currentLang)} invalid={invalid}>
            <Select
              ...
              data={data}
              field={field}
              placeholder={lang.tr(field.placeholder)}
              onChange={(value) =>
                dispatch({
                  type: 'updateField',
                  data: {
      ...
      case 'text_array':
        return (
          <TextFieldsArray
            ...
            addBtnLabel={lang.tr(field.group?.addLabel)}
            addBtnLabelTooltip={lang.tr(field.group?.addLabelTooltip)}
          />
        )
  ...
  return (
    <Fragment>
      ...
      {!!advancedSettings?.length && (
        <GroupItemWrapper defaultCollapsed borderTop={!!fields.length} label={lang.tr('advancedSettings')}>
